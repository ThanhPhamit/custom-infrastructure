...
# ALB 5xx alarms
#
# ALB ONLY: HTTPCode_* metrics live in the AWS/ApplicationELB namespace only.
# AWS/NetworkELB does not publish them, so both alarms are skipped when
# load_balancer_type != "alb".
#
# Why these are needed on top of the log-error alarm above: 5xx generated by the
# load balancer itself (target died mid-request, stale keep-alive connection,
# target timeout) never reach the ECS log group, so a log-pattern alarm is blind
# to them.
#
#   HTTPCode_ELB_5XX_Count    -> generated BY the ALB   (the target never answered)
#   HTTPCode_Target_5XX_Count -> returned BY the target (the application itself)
#
# Keeping the two apart is what makes triage fast: ELB_5XX > 0 while
# Target_5XX == 0 means the app never answered at all (crash / OOM-kill /
# connection reset), NOT an application-level error.
# ---------------------------------------------------------------------------

resource "aws_cloudwatch_metric_alarm" "lb_elb_5xx" {
  count = var.create_lb_5xx_alarm && var.load_balancer_type == "alb" ? 1 : 0

  alarm_name          = "${var.app_name}_alb_elb_5xx"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = var.cw_alarm_lb_5xx_evaluation_periods
  metric_name         = "HTTPCode_ELB_5XX_Count"
  namespace           = "AWS/ApplicationELB"
  period              = var.cw_alarm_lb_5xx_period
  statistic           = "Sum"
  threshold           = var.cw_alarm_lb_elb_5xx_threshold
  alarm_description   = "ALB-generated 5xx (target died mid-request, stale keep-alive connection, target timeout). These never reach the ECS log group, so the log-error alarm cannot see them."
  alarm_actions       = [var.chatbot_alert_sns_topic_arn]
  ok_actions          = [var.chatbot_alert_sns_topic_arn]
  ...
  tags = merge(
    var.tags,
    {
      Name = "${var.app_name}-alb-elb-5xx"
    }
  )
}

resource "aws_cloudwatch_metric_alarm" "lb_target_5xx" {
  count = var.create_lb_5xx_alarm && var.load_balancer_type == "alb" ? 1 : 0

  alarm_name          = "${var.app_name}_alb_target_5xx"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = var.cw_alarm_lb_5xx_evaluation_periods
  ...
  period              = var.cw_alarm_lb_5xx_period
  statistic           = "Sum"
  threshold           = var.cw_alarm_lb_target_5xx_threshold
  alarm_description   = "Targets returned 5xx responses (application-level errors). A healthy service should sit at zero."
  alarm_actions       = [var.chatbot_alert_sns_topic_arn]
  ok_actions          = [var.chatbot_alert_sns_topic_arn]
  treat_missing_data  = "notBreaching"
