...
variable "azs_name" {
  description = "A list of availability zone suffixes (e.g., ['a', 'b', 'c'])"
  type        = list(string)
  default     = []
}
...
variable "create_elasticache_nat_gateway_route" {
  description = "Controls if a nat gateway route should be created to give internet access to the elasticache subnets"
  type        = bool
  default     = false
}
