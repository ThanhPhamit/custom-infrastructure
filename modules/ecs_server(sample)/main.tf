...
# =============================================================================
# SECRETS — container only (no value managed by Terraform)
# =============================================================================
# Terraform creates ONLY the empty secret container. The JSON value is uploaded
# out-of-band (Secrets Manager console, or a gitignored secrets/<app>.json that
# you paste in). Terraform never generates or reads a secret value, so NOTHING
# sensitive is stored in Terraform state.
#
# ONE secret holds ALL keys (managed manually — generate with e.g. `openssl rand`):
...
resource "aws_ecs_task_definition" "task_definition" {
  ...
  container_definitions = templatefile("${path.module}/container_definitions/server-task-def.json.tpl", {
    ...
    # Single app secret container (value managed out-of-band). ALL secret env
    # vars read from this one secret via "<arn>:<key>::".
    app_runtime_secret_arn = aws_secretsmanager_secret.app_secrets.arn
  })
