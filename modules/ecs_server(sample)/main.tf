...
# These values are generated by Terraform and stored in one JSON secret
# =============================================================================
...
# Read DB password value from the RDS-managed secret ARN.
data "aws_secretsmanager_secret_version" "postgres_password" {
  secret_id = var.postgres_password_secret_arn
}

# Store all runtime application credentials in one JSON secret.
resource "aws_secretsmanager_secret" "app_runtime_secrets" {
  name = "${var.app_name}-ecs-runtime-secrets-${substr(random_uuid.ecs_secrets_uuid.result, 0, 4)}"

  tags = merge(var.tags, {
    Name = "${var.app_name}-ecs-runtime-secrets"
  })
}

resource "aws_secretsmanager_secret_version" "app_runtime_secrets" {
  secret_id = aws_secretsmanager_secret.app_runtime_secrets.id
  secret_string = jsonencode({
    admin_secret_key       = random_password.admin_secret_key.result
...
resource "aws_ecs_task_definition" "task_definition" {
  ...
  container_definitions = templatefile("${path.module}/container_definitions/server-task-def.json.tpl", {
    ...
    # Consolidated runtime app secrets (single secret)
    app_runtime_secret_arn = aws_secretsmanager_secret.app_runtime_secrets.arn
    email_smtp_secret_arn  = var.email_smtp_secret_arn
  })
